...
- Invoke the SDK
  ...
  ```Javascript
   Cordova.exec(successCallBack, failureCallBack, "OCTUS", "doOCR", [scanSDKParams]);
  ```
...
Example:  `cordova create octusSDKTest com.octussdktest octusApp`
                 `cd octusSDKTest`
...
Step 01:
- Extract the plugin ZIP file :  `FRSOctusSDKIOS.zip`

Step 02:
- Add the swift support to cordova app :  `cordova plugin add cordova-plugin-add-swift-support --save`

Step 03:
- Add the plugin to your app : `cordova plugin add /path/to/FRSOctusSDKIOS`
...
- Create callback methods
    ```Javascript
        var successCallBack = function(result){
            // You get the result of the JSONString reponse from the captured document along with an URLpath, which is located in document directory.
            console.log(result);
        }
    ```
...
- Invoke the plugin
    ```Javascript
       FRSOctusSDKIOS.invokeOctusSDK(inputParamsDict,successCallback,errorCallback);
    ```
